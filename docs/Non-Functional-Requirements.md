...
| Server-side HTTP Cache | ✅ | `apps/backend/src/search/search.service.ts` | `CacheService` sử dụng Redis để cache kết quả Search API (TTL 10s) |
...
| Redis-backed Rate Limiter | ✅ | `apps/backend/src/app.module.ts` | `ThrottlerStorageRedisService` đồng bộ rate-limit giữa các instances |
...
## 8. Maintainability & Code Quality (Tính bảo trì)

| Feature | Status | Location | Description |
|---------|--------|----------|-------------|
| Monorepo Architecture | ✅ | `package.json` (root) | Sử dụng npm workspaces chia tách rõ ràng `apps/frontend`, `apps/backend`, `packages/shared`. |
| TypeScript Strict Mode | ✅ | `tsconfig.json` | 100% codebase được type-check chặt chẽ (`strict: true`), ngăn chặn lỗi runtime. |
| Shared Types/Constants | ✅ | `packages/shared/` | Frontend và Backend dùng chung định nghĩa DTO và Schema, đảm bảo Data Consistency. |
| Code Formatter & Linter | ✅ | `.eslintrc`, `.prettierrc` | Đảm bảo code style đồng nhất trên toàn dự án trước khi commit. |
